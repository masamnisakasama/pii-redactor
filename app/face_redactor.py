...
def _detect_faces_haar(bgr: np.ndarray,
    ...
    base = os.getenv("OPENCV_MODEL_PATH", cv2.data.haarcascades)
    all_faces = []
    for cascade_file in cascade_files:
        try:
            cc = cv2.CascadeClassifier(os.path.join(base, cascade_file))
            if cc.empty():
                continue
...
# 中心ほど強くブラー、元が誰かわからないように
def _smart_blur_roi(img: np.ndarray, x: int, y: int, w: int, h: int, strength: int = 25):
    """
    スマートブラー（中心強・周辺中）の強化版
    - 顔の中心ほど強く、外側はやや弱めに。
    - いずれも σ は顔サイズ比例。
    """
    roi = img[y:y+h, x:x+w]
    shortest = max(1, min(w, h))
    base = max(3.0, shortest * (strength / 100.0) * 0.45)

    light  = cv2.GaussianBlur(roi, (0, 0), sigmaX=base * 0.7, sigmaY=base * 0.7)
    mid    = cv2.GaussianBlur(roi, (0, 0), sigmaX=base * 1.0, sigmaY=base * 1.0)
    heavy  = cv2.GaussianBlur(roi, (0, 0), sigmaX=base * 1.8, sigmaY=base * 1.8)

    # 中央楕円マスク（フェザー広め）
    mask = np.zeros((h, w), dtype=np.uint8)
    cv2.ellipse(mask, (w//2, h//2), (int(w*0.42), int(h*0.42)), 0, 0, 360, 255, -1)
    mask = cv2.GaussianBlur(mask, (0, 0), sigmaX=max(4, int(shortest * 0.06)))
    m = (mask.astype(np.float32) / 255.0)[..., None]  # 0..1, 3ch

    # 中心は heavy、外側は mid を基本に、全体に light をほんのり混ぜる
    out = (heavy * m + mid * (1 - m)).astype(np.uint8)
    out = cv2.addWeighted(out, 0.85, light, 0.15, 0)
...
def _blur_roi_advanced(img: np.ndarray, x: int, y: int, w: int, h: int, k: int = 51):
    """
    改良されたブラー処理（σを顔サイズと強度に比例させる）
    - strength は 0..100 相当で解釈（呼び出し側ではそのまま int を渡してOK）
    - かなり強めにぼかす。顔が大きいほど強くなる。
    """
    roi = img[y:y+h, x:x+w]

    # strength を 0..1 に正規化し、顔最短辺に比例させて σ を決める
    # 係数 0.55 は体感で“強め”になるようにチューニング
    shortest = max(1, min(w, h))
    s = max(3.0, shortest * (k / 100.0) * 0.55)

    # (0,0) を渡すとカーネルサイズは自動決定、σだけで強度が決まる
    blurred = cv2.GaussianBlur(roi, (0, 0), sigmaX=s, sigmaY=s)

    # さらにエッジをならすため軽いメディアンを重ねる
    med_k = int(max(3, (s // 4))) | 1
    blurred = cv2.medianBlur(blurred, med_k)
...
def _oval_face_mask(
    w: int,
    h: int,
    scale_x: float = 0.76,   # 横をややタイトに
    scale_y: float = 0.84,   # 縦も少しタイトに
    upshift: float = 0.06,   # マスク中心を少し上へ（顎・首を巻き込みにくく）
    feather_ratio: float = 0.10,
) -> np.ndarray:
    """
    顔楕円マスク（0-255）。矩形ROI全体ではなく“顔内部”中心に効かせる。
    """
    cx, cy = w // 2, int(h * (0.5 - upshift))
    rx, ry = int(w * 0.5 * scale_x), int(h * 0.5 * scale_y)
    mask = np.zeros((h, w), dtype=np.uint8)
...
# 輪郭を綺麗にするアルファブレンド関数
def _alpha_over(dst_bgr: np.ndarray, src_bgr: np.ndarray, alpha01: np.ndarray) -> np.ndarray:
    """dstにsrcをalpha(0..1)で重ねる"""
    a = alpha01[..., None].astype(np.float32)
    return (a * src_bgr.astype(np.float32) + (1 - a) * dst_bgr.astype(np.float32)).clip(0, 255).astype(np.uint8)
...
_genai_client_key: Optional[str] = None  # 直近で使ったAPIキーを保持

def _get_genai_client(explicit_api_key: Optional[str] = None) -> Optional["genai.Client"]:
    """
    google-genai のクライアントを返す（キーが無いときは静かに None）。
    - 明示キー > 環境変数 GOOGLE_API_KEY / GENAI_API_KEY
    - キーが変わったらクライアントを作り直し
    - ライブラリが無ければ None
    """
    global _genai_client_cache, _genai_client_key

    if not _GENAI_AVAILABLE:
        return None

    # キーの解決（どれも無ければオフライン扱いで終了）
    api_key = explicit_api_key or os.getenv("GOOGLE_API_KEY") or os.getenv("GENAI_API_KEY")
    if not api_key:
        return None

    # まだ未生成 or キー変更 → 作り直し
    if _genai_client_cache is None or api_key != _genai_client_key:
        try:
            _genai_client_cache = genai.Client(api_key=api_key)
            _genai_client_key = api_key
        except Exception as e:
            # オフラインん導入したので通常黙るように変更　デバッグ時だけメッセージ出す予定。
            if os.getenv("DEBUG_GENAI"):
...
async def redact_faces_image_bytes_enhanced(
    ...
    from .detectors import _post_filter  # ← 関数内で！　循環回避のため！
    faces = _post_filter(faces, W, H)    # ← 誤検知を間引く仕組み
    ...
    for (x, y, w, h) in faces:
         # blur 系は expand を広げすぎない（肌色強すぎるの抑制）
        eff_expand = expand
        if method in ("blur", "smart_blur"):
            eff_expand = min(expand, 0.25)
        padw, padh = int(w * eff_expand), int(h * eff_expand)
        ...
        # Haar輪郭残るので輪郭ごと丸ごと消してpixalate　strictでなくてももう消す
        if method == "pixelate":
            work = roi.copy()
            _pixelate_roi_advanced(work, 0, 0, w0, h0, blocks=max(6, int(strength)))
            alpha = _oval_face_mask(w0, h0).astype(np.float32) / 255.0
            bgr[y0:y0+h0, x0:x0+w0] = _alpha_over(roi, work, alpha)
        ...
        # Haar輪郭残るので輪郭ごと丸ごと消してpixalate
            ...
            work = roi.copy()
            _blur_roi_advanced(work, 0, 0, w0, h0, k=max(7, int(strength)))
            alpha = _oval_face_mask(w0, h0).astype(np.float32) / 255.0
            bgr[y0:y0+h0, x0:x0+w0] = _alpha_over(roi, work, alpha)
            ...
            work = roi.copy()
            _smart_blur_roi(work, 0, 0, w0, h0, strength=max(7, int(strength)))
            alpha = _oval_face_mask(w0, h0).astype(np.float32) / 255.0
            bgr[y0:y0+h0, x0:x0+w0] = _alpha_over(roi, work, alpha)
